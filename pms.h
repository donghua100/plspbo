...
void Satlike::build_neighbor_relation()
{
	...
	for (v = 1; v <= num_vars; ++v)
	{
		...
		count = 0;
		// printf("var: %d neighbor: ", v);
		for (i = 0; i < temp_neighbor_count; i++)
		{
			// printf("%d ",temp_neighbor[i]);
...
void Satlike::init(vector<int> &init_solution)
{
	...
	printf("%-20s %-20s %-20s %-20s %-20s\n","clause type","org_clause_weight", "clause_weight", "org_unit_weight", "unit_weight");
	for (c = 0; c < num_clauses; c++)
	{
		...
		else
		{
			...
			unit_weight[c] = org_unit_weight[c];
			printf("%-20s ","soft clause");
		}
		printf("%-20lld %-20lld %-20lld %-20lld\n", org_clause_weight[c], clause_weight[c], org_unit_weight[c], unit_weight[c]);
	}
